...
> ⚠️ **Important:** Many online tutorials and blog posts contain incorrect schema patterns for Magento 2.4.8 message queues. The following corrections are verified against the actual XSD schemas.

| Mistake (wrong pattern) | Correct Pattern | Source |
|---|---|---|
| DLQ config in `queue_topology.xml` using `<DLQ>`, `<max-retries>`, `<retry-delay-ms>` | DLQ settings belong in `env.php` under `queue.dead_letter_queue` — these XML elements do not exist in the async topology XSD | `env.php` |
| Wrong consumer XSD: `urn:magento:framework:MessageQueue/etc/consumer.xsd` | Correct: `urn:magento:framework:Async/Consumer/etc/consumer.xsd` | `queue_consumer.xml` |
| Using `<DLQ>` element in `queue_topology.xml` | DLQ is configured via `env.php` only — the `<DLQ>` element does not exist in `queue_topology.xsd` | `env.php` |
| `connection="rabbitmq"` in `queue_topology.xml` | Use `connection="amqp"` — RabbitMQ uses the AMQP protocol adapter | `queue_topology.xml` |
| Defining `maxMessages` as an unlimited buffer | Always set `maxMessages` to a finite value (e.g. `1000`) to prevent memory exhaustion | `queue_consumer.xml` |

### Why `<max-retries>` Does Not Exist

The `<max-retries>` and `<retry-delay-ms>` elements appear in community tutorials but are **not declared** in `urn:magento:framework:Async/MessageQueue/etc/async_topology.xsd`. The dead letter queue behavior (number of retries before a message goes to DLQ) is controlled by consumer configuration in `env.php`:
...
return [
    'queue' => [
        'dead_letter_queue' => [
            'max_retries' => 3,        // retries before DLQ — NOT in XML
            'retry_delay' => 5000,     // ms between retries — NOT in XML
        ],
    ],
];
```

### Correct Schema Locations

| File | Correct XSD |
|---|---|
| `queue_topology.xml` | `urn:magento:framework:Async/MessageQueue/etc/async_topology.xsd` |
| `queue_consumer.xml` | `urn:magento:framework:Async/Consumer/etc/consumer.xsd` |
| `communication.xml` | `urn:magento:framework:Communication/etc/communication.xsd` |

For a detailed treatment of async-only AMQP topology configuration and the complete corrected schema patterns, see **[21-async-message-queue.md](./21-async-message-queue.md)**.
