...
**Important:** Rate limiting in M2.4.8 is configured via `di.xml` dependency injection. It is NOT configured via `webapi.xml` or any topology XML. The rate limiter classes (`RateLimiterInterface`, `CustomerRateLimiter`, etc.) are registered and their limits set via `<type>` arguments in `di.xml`.
